...
with col2:
    stocks_count = (
        len(holdings_df[holdings_df["type"] == "stock"]) if not holdings_df.empty else 0
    )
    st.metric("Stocks", stocks_count)

with col3:
    mf_count = (
        len(holdings_df[holdings_df["type"] == "mutual_fund"])
...
with st.expander("Preview Holdings Data"):
    ...
    with tab1:
        stocks_df = (
            holdings_df[holdings_df["type"] == "stock"]
            if not holdings_df.empty
            else pd.DataFrame()
        )
    ...
    with tab2:
        mf_df = (
            holdings_df[holdings_df["type"] == "mutual_fund"]
            if not holdings_df.empty
            else pd.DataFrame()
        )
        if not mf_df.empty:
