...
def create_portfolio_excel(
    ...
    stocks_df = (
        holdings_df[holdings_df["type"] == "stock"].copy()
        if not holdings_df.empty
        else pd.DataFrame()
    )
    _create_holdings_sheet(
        ws_stocks, stocks_df, "Stocks Holdings", header_font, header_fill, thin_border
    )

    # Sheet 3: Mutual Funds
    ws_mf = wb.create_sheet("Mutual Funds")
    mf_df = (
        holdings_df[holdings_df["type"] == "mutual_fund"].copy()
        if not holdings_df.empty
        else pd.DataFrame()
    )
    _create_holdings_sheet(
        ws_mf, mf_df, "Mutual Funds Holdings", header_font, header_fill, thin_border
    )
